...
Webhooks provide real-time notifications for various events in the payment system:

- **Payment Events** - Deposit confirmations, withdrawal status updates
- **Transaction Events** - Transaction status changes and completions
- **Settlement Events** - Settlement approvals and processing updates
- **System Events** - Balance updates, fee changes, and system notifications

## Webhook Setup

### Configuring Your Webhook Endpoint

To receive webhooks, you need to configure a webhook URL in your account settings. The webhook endpoint should:

1. **Accept POST requests** with JSON payloads
2. **Return 200 status** for successful processing
3. **Handle signature verification** for security
4. **Implement idempotency** to prevent duplicate processing

### Webhook URL Configuration

```bash
# Configure webhook URL (via admin panel or API)
POST /api/admin/system-settings
{
  "key": "webhook_url",
  "value": "https://your-domain.com/webhooks/dce",
  "category": "webhooks"
}
```
...
The dce API sends webhooks for the following event types:

| Event Type             | Description                     | Payload Structure                         |
| ---------------------- | ------------------------------- | ----------------------------------------- |
| `deposit.confirmed`    | Deposit payment confirmed       | Deposit data with confirmation details    |
| `deposit.failed`       | Deposit payment failed          | Deposit data with failure reason          |
| `deposit.pending`      | Deposit payment pending         | Deposit data with pending status          |
| `payout.confirmed`     | Withdrawal payment confirmed    | Withdrawal data with confirmation details |
| `payout.failed`        | Withdrawal payment failed       | Withdrawal data with failure reason       |
| `payout.pending`       | Withdrawal payment pending      | Withdrawal data with pending status       |
| `settlement.approved`  | Settlement request approved     | Settlement data with approval details     |
| `settlement.completed` | Settlement processing completed | Settlement data with completion details   |
| `settlement.failed`    | Settlement processing failed    | Settlement data with failure reason       |
...
```json
{
  ...
  "correlationId": "webhook-1752630133651-y2xukk82i",
  "data": {
    // Event-specific data
  },
  "metadata": {
    "merchantId": "merchant_xxxxxxxxxxxxxxxx",
...
The webhook endpoint receives and processes incoming webhook events.
...
User-Agent: dce-webhook/1.0
X-Webhook-ID: webhook-1752630133651-y2xukk82i
```
...
```json
{
  "status": "received",
  "correlationId": "webhook-1752630133651-y2xukk82i",
  "processedAt": "2024-12-19T10:30:05Z"
}
```
...
All webhooks include a cryptographic signature to verify authenticity:
